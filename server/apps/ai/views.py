...
try:
    ...
    try:
        ...
        log.warning("Could not import research_market from research or researcher: %s", e)
...
def _ok(bot: str, done: bool = False, results_url: str | None = None, extra: Dict[str, Any] | None = None):
...
def api_message(request):
    ...
    if state.get("state") == "COMPLETE":
        return _ok("Interview completed. Open the Results page.", done=True, results_url="/results")
    ...
    if user_text.upper() == "RESULTS":
        if research_market is None:
            return JsonResponse({"bot": "Research module not found. Please ensure research.py or researcher.py is present.",
                                 "done": False}, status=500)
        try:
            # final sync turn to capture any last updates (empty text = no change)
            ...
            plan_md, _preview = generate_markdown_plan(
                answers=answers, research=research, template_id=template_id, language=lang
            )
            state["last_plan"] = plan_md
            state["state"] = "COMPLETE"
            return _ok("Done. Your plan is ready—open the Results page.", done=True, results_url="/results")
        except Exception as e:
            log.exception("plan generation failed: %s", e)
            return JsonResponse({"bot": "Sorry—failed to generate the plan.", "error": str(e)}, status=500)
    ...
    try:
        ...
        _merge_answers(answers, updates)
        if not next_q or (history and history[-1]["role"] == "assistant" and history[-1]["content"].strip() == next_q.strip()):
...
def _safe_next_turn(answers: Dict[str, str], user_text: str, lang: str) -> Tuple[Dict[str, str], str, bool]:
    """
