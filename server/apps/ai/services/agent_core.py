...
Produce full professional business plans in Markdown that STRICTLY follow a given
section outline.
- Spain focus: market sizing in EUR, competitors operating in Spain,
  Spanish/EU regulations.
...
def _mk_inputs_block(answers: Dict[str, Any]) -> str:
    if not answers:
        return "(no structured answers provided)"
    out = []
...
def _mk_research_block(research: Dict[str, Any]) -> str:
    ...
    if facts:
        parts.append(
            "**Facts:** "
            + " | ".join(f"{f['fact']} (src: {f['source']})" for f in facts[:8])
        )
    if srcs:
        parts.append(
            "**Sources:** "
            + " | ".join(f"{s['title']} ({s['url']})" for s in srcs[:8])
        )
    return "\n".join(parts)
...
- GTM: at least 4 channels relevant in Spain; include monthly launch calendar
  for first year.
- Regulations: note Spain/EU compliance relevant to the industry (e.g., GDPR,
  sector licenses).
...
""".strip()
    ...
    preview.append(
        "Includes: Spain competitors, EUR sizing, GTM, 12-month P&L, regulations."
    )
    return md, "\n".join(preview)
...
def generate_plans(
    ...
    referee=True,
    research=None,
    template_id="template_a_ministry",
):
    if research is None:
        research = research_market(answers)
    md, preview = generate_markdown_plan(
        answers, research, template_id=template_id, language=language
    )
    return md, preview
